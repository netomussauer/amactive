...
describe('Sidebar', () => {
  ...
  it('ADMIN vê todos os itens de navegação', () => {
    ...
    for (const label of ['Dashboard', 'PDV', 'Pedidos', 'Produtos', 'Estoque', 'Clientes', 'Fornecedores', 'Relatórios']) {
      expect(screen.getByRole('link', { name: label })).toBeInTheDocument()
    }
  })

  it('VENDEDOR não vê Dashboard, Fornecedores nem Relatórios', () => {
    login('VENDEDOR')
    renderSidebar()
    ...
    for (const label of ['Dashboard', 'Fornecedores', 'Relatórios']) {
      expect(screen.queryByRole('link', { name: label })).not.toBeInTheDocument()
    }
  })

  it('ESTOQUISTA não vê Dashboard, PDV, Clientes nem Relatórios', () => {
    login('ESTOQUISTA')
    renderSidebar()
    ...
    for (const label of ['Dashboard', 'PDV', 'Clientes', 'Relatórios']) {
      expect(screen.queryByRole('link', { name: label })).not.toBeInTheDocument()
    }
  })
